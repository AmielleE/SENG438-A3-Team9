...
| Class | Statement (Instruction) | Decision (Branch) | Method |
|----------|----------|----------|----------|
| Range    | 60.9%     | 52.4%     | 72.9%     |
| DataUtilities    | 48.0%     | 29.7%     | 50.0%     |
...
Coverage As, then JUnit Test, which produced:
...
- It can be misleading. Like a statement can execute without verifying correctness (weak or missing assertions).
- 100% statement coverage does not guarantee all decision outcomes were tested.

Pros of Decision Coverage: 
- Stronger than statement coverage. Like it requires tests that exercise both outcomes of decisions (like true/false paths).
- Useful for finding missing edge-case tests, especially around validation and exception branches.

Cons of Decision Branch Coverage:
- Still does not guarantee full logical coverage. Like complex conditions can still hide untested sub-conditions.
...
- Very coarse metric, so it only checks if a method was entered.
...
The method also contains a second loop with the condition r2 > rowCount. Since r2 is initialized to 0 and rowCount is always non-negative, the condition r2 > rowCount can't be satisfied during the first run through. Therefore, the second loop is logically unreachable and represents an infeasible execution path.
...
data was defined at method entry and used in null check, getRowCount(), and getValue().
column was also defined at method entry, and used in getValue(r, column).
total was defined at initialization, used in accumulation and return statement, and was redefined inside the loop when values are added.
rowCount was defined using data.getRowCount(), and was used in the loop condition.
r was defined in the for loop and used as an index in getValue(r, column). 
n was defined when retrieving each cell value and used in the null check and in the addition to total. 
Only feasible DU-pairs are considered because the second loop is unreachable.
data was defined at entry to be used in null check, then used in getRowCount(), and finally used in getValue().
...
Therefore the DU-pair coverage for DataUtilities.calculateColumnTotal is about 100% for all feasible paths.  
...
value was defined at method entry and used in contains(value) and comparison conditions.
result was defined initially as value, then possibly redefined if the value is outside the bounds, and used in the return statement.
lower and upper were used in conditional comparisons to determine constraint behavior.
...
Therefore, the DU-pair coverage for Range.constrain is 100 % since all feasible definition use paths for the result variable are exercised by the test suite.
...
In a lot of cases, we specifically designed new tests to trigger previously uncovered branches identified in the coverage report, like boundary conditions in the Range class and null-handling logic in DataUtilities.
...
Overall, coverage tools are useful for identifying gaps in test suites and guiding additional test design, but they must be used together with well-designed assertions and requirement-based tests to make sure everything is correct.
...
- Fatma & Amielle designed new unit tests for org.jfree.data.Range to increase code coverage.
...
Overall, the work was coordinated through collaboration and task sharing, enabling the team to make progress efficiently while keeping clear records of results and defects.
